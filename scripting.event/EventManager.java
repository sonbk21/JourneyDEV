...
public class EventManager {
    ...
    public List<EventRecordEntry> getWorldRecords() { //New functions, related to record system.
        return cserv.getRecordsManager().getRecords(name);
    }
    ...
    public String getNpcTextRankings() {
        ...
        if (getWorldRecords().isEmpty())
            loaded = cserv.getRecordsManager().loadRecords(name);
        if (loaded) {
            List<EventRecordEntry> records = getWorldRecords();
            StringBuilder sb = new StringBuilder();
            int i = 0;
            for (EventRecordEntry ere : records) {
                sb.append(String.valueOf(i+1));
                sb.append(". ");
                sb.append(String.valueOf((int) Math.floor(ere.getTime()/60)));
                sb.append(":");
                if (ere.getTime()%60 < 10)
                    sb.append("0");
                sb.append(String.valueOf(ere.getTime()%60));
                sb.append(" by ");
                sb.append(ere.getNames());
                sb.append("\r\n");
                i++;
            }
            return sb.toString();
