...
public class EventInstanceManager {
    ...
    public int setRecord(int time) {
        List<EventRecordEntry> records = em.getWorldRecords();
        boolean loaded = true;
        
        if (records.isEmpty())
            loaded = em.getWorldRecordsManager().loadRecords(em.getName());
        
        if (loaded) {
            for (EventRecordEntry ere : records) {
                if (time < ere.getTime()) {
                    if (em.getWorldRecordsManager().updateRecords(em.getName(), getPartyNames(), time, records.indexOf(ere)))
            ...
            if (records.size() > 14)
                return 0;
            em.getWorldRecordsManager().updateRecords(em.getName(), getPartyNames(), time, records.size());
            return records.size() + 1;
        } else {
            em.getWorldRecordsManager().updateRecords(em.getName(), getPartyNames(), time, 0);
            return 1;
        }
    }
