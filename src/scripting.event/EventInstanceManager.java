...
public class EventInstanceManager {
    ...
    public int setRecord(int time) {
        ...
        if (records == null)
            loaded = em.getWorldRecordsManager().loadRecords(RecordEvent.valueOf(em.getName()));
        
        if (loaded) {
            for (Pair<String, Integer> ere : records) {
                if (time < ere.getRight()) {
                    if (em.getWorldRecordsManager().updateRecords(RecordEvent.valueOf(em.getName()), getPartyNames(), time, records.indexOf(ere)))
            ...
            if (records.size() > 14)
                return 0;
            em.getWorldRecordsManager().updateRecords(RecordEvent.valueOf(em.getName()), getPartyNames(), time, records.size());
            return records.size() + 1;
        } else {
            em.getWorldRecordsManager().updateRecords(RecordEvent.valueOf(em.getName()), getPartyNames(), time, 0);
            return 1;
        }
    }
