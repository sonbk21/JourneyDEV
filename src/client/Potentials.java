    ...
    public void resetStats(MapleCharacter chr, boolean add) {
        ...
        if (add) {
            for (PotentialLine line : lines) {
                ...
                int tmp = 0;
                switch (line.getEffect()) {
                    case STR: tmp += (value / 100) * chr.getStat(MapleStat.STR); break;
                    case DEX: tmp += (value / 100) * chr.getStat(MapleStat.DEX); break;
                    case INT: tmp += (value / 100) * chr.getStat(MapleStat.INT); break;
                    case LUK: tmp += (value / 100) * chr.getStat(MapleStat.LUK); break;
                ...
                if (tempstats.containsKey(line.getEffect()))
                    tempstats.put(line.getEffect(), tmp + tempstats.get(line.getEffect()));
                else
                    tempstats.put(line.getEffect(), tmp);
            }
            addStats(chr);
        }
    }
