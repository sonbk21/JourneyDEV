...
public class MapleClient {
    ...
    public int getVPoints() {
        ...
        ResultSet rs = null;
        try {
            ps = con.prepareStatement("SELECT vpoints FROM accounts WHERE name = ?");
    ...
    public void setVPoints(int vpoints) {
        ...
        Connection con = DatabaseConnection.getConnection();
        try {
            try (PreparedStatement ps = con.prepareStatement("UPDATE accounts SET vpoints = ? WHERE id = ?")) {
                ps.setInt(1, this.vpoints);
                ps.setInt(2, accId);
                ps.executeUpdate();
            }
        } catch (SQLException e) {
    ...
    public int login(String login, String pwd) {
        ...
        ResultSet rs = null;
        try {
            ps = con.prepareStatement("SELECT id, password, salt, gender, banned, gm, pin, pic, characterslots, tos, vpoints FROM accounts WHERE name = ?");
    ...
    public final void disconnect(boolean voluntarily, boolean cashshop) {//once per MapleClient instance
        if (disconnecting) {
            return;
        }
        disconnecting = true;
    ...
    public boolean deleteCharacter(int cid) {
        ...
        try {
            try (PreparedStatement ps = con.prepareStatement("SELECT id, guildid, guildrank, name, allianceRank FROM characters WHERE id = ? AND accountid = ?")) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    ...
                    if (rs.getInt("guildid") > 0) {
                        try {
                            Server.getInstance().deleteGuildCharacter(new MapleGuildCharacter(cid, (short) 0, rs.getString("name"), (byte) -1, (byte) -1, (short) 0, rs.getInt("guildrank"), rs.getInt("guildid"), false, rs.getInt("allianceRank")));
                        } catch (Exception re) {
                            return false;
                        }
                    }
            ...
            String[] toDel = {"famelog", "inventoryitems", "keymap", "queststatus", "savedlocations", "skillmacros", "skills"};
            for (String s : toDel) {
    ...
    public synchronized void showEffect(String path) {
        session.write(MaplePacketCreator.showEffect(path));
    }
    
    public synchronized void showEnergy() {
        session.write(MaplePacketCreator.getEnergy("energy", getPlayer().getDojoEnergy()));
    }
}
