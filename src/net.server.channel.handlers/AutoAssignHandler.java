...
public class AutoAssignHandler extends AbstractMaplePacketHandler {
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            short tempVal = (short) slea.readInt();
    ...
    private int gainStatByType(MapleCharacter chr, MapleStat type, short gain) {
        short newVal = (short) (chr.getStat(type) + gain);
        AbilityStat temp = AbilityStat.values()[type.ordinal() - 5];
        if (newVal > 999 + chr.getAbilities().getTempStat(temp))
            chr.setStat(type, (short) (999 + chr.getAbilities().getTempStat(temp)));
        else
