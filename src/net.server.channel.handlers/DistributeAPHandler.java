...
public final class DistributeAPHandler extends AbstractMaplePacketHandler {
    ...
    static boolean addStat(MapleClient c, int id) {
        ...
        if (id < 2048) {
            max += c.getPlayer().getAbilities().getTempStat(AbilityStat.values()[type.ordinal() - 5]);
            if (c.getPlayer().getStat(type) >= max) {
                return false;
            }
            c.getPlayer().addStat(type, (short) 1);
        } else if (id == 2048) {
    ...
    static short addHP(MapleClient c) {
        MapleCharacter player = c.getPlayer();
        MapleJob job = player.getMapleJob();
        short MaxHP = player.getStat(MapleStat.MAXHP);
    ...
    static short addMP(MapleClient c) { //isnt this function also in maplecharacter?..
        MapleCharacter player = c.getPlayer();
        short MaxMP = player.getStat(MapleStat.MAXMP);
        MapleJob job = player.getMapleJob();
    ...
    static void addHP(MapleCharacter player, short up) {
        if (up > 30000)
            up = 30000;
        player.setHpMpApUsed(player.getHpMpApUsed() + 1);
        player.setStat(MapleStat.MAXHP, up);
    }

    static void addMP(MapleCharacter player, short up) {
        if (up > 30000)
            up = 30000;
        player.setHpMpApUsed(player.getHpMpApUsed() + 1);
        player.setStat(MapleStat.MAXMP, up);
    }
}
